...
public class Subsume {
    ...
    public Subsume(final InstrumentOptions options) {
        this.src = options.getSource();
        this.dest = options.getDestination();
    }

    public int subsume() throws IOException {
        ...
        for (final File file : files) {
            n += subsume(file, new File(relativize(dest)));
        }
        return n;
    }

    private int subsume(final File src, final File dest) throws IOException {
        ...
        try {
            return Subsumer.subsumeAll(input, dest.getPath());
        } finally {
            input.close();
        }
    }
    ...
    public static void main(final String[] args) {
        ...
        try {
            ...
            final int total = new Subsume(options).subsume();
            final long seconds = tw.time(TimeUnit.SECONDS);

            System.out.println(MessageFormat.format(
                    "{0} methods had the subsumption relation calculated in {1} minutes and {2} seconds", total, seconds/60, seconds % 60));
